 ...
 */
static void *scmi_memory_retrieve(ffa_id_t source, ffa_id_t vm_id,
				  uint64_t handle, struct mailbox_buffers *mb)
{
...
/**
 * Parse the command and invokes the respective handler.
 */
bool scmi_handle_cmd(struct ffa_value *cmd_args, struct ffa_value *ret,
		       struct mailbox_buffers *mb)
{
	...
	in_cmd = (uint64_t)ret->arg3;

	switch (in_cmd) {
	case FFA_SCMI_CMD_CAPABILITIES:
		*ret = ffa_msg_send_direct_resp32(ffa_dir_msg_dest(*cmd_args),
	...
	*/
	...
	for (;;) {
		...
		if ((ffa_func_id(ffa_ret) == FFA_INTERRUPT) ||
			 ...
			 *
			 * TODO: check for pending INDIRECT message.
			 */
			ffa_ret = ffa_msg_wait();
			continue;
		}
		...
		if (!scmi_handle_cmd(&ffa_ret, &ffa_ret, mb)) {
			continue;
		}
	}
...
static void scmi_print_memory_layout(unsigned int vm_id)
{
	INFO("Secure Partition memory layout:\n");

	INFO("  Text region            : %p - %p\n",
		(void *)SCMI_TEXT_START, (void *)SCMI_TEXT_END);

	INFO("  Read-only data region  : %p - %p\n",
		(void *)SCMI_RODATA_START, (void *)SCMI_RODATA_END);

	INFO("  Data region            : %p - %p\n",
		(void *)SCMI_DATA_START, (void *)SCMI_DATA_END);

	INFO("  BSS region             : %p - %p\n",
		(void *)SCMI_BSS_START, (void *)SCMI_BSS_END);
...
static void scmi_plat_configure_mmu(unsigned int vm_id)
{
	mmap_add_region(SCMI_TEXT_START,
			SCMI_TEXT_START,
			SCMI_TEXT_END - SCMI_TEXT_START,
			MT_CODE);
	mmap_add_region(SCMI_RODATA_START,
			SCMI_RODATA_START,
			SCMI_RODATA_END - SCMI_RODATA_START,
			MT_RO_DATA);
	mmap_add_region(SCMI_DATA_START,
			SCMI_DATA_START,
			SCMI_DATA_END - SCMI_DATA_START,
			MT_RW_DATA);
	mmap_add_region(SCMI_BSS_START,
			SCMI_BSS_START,
			SCMI_BSS_END - SCMI_BSS_START,
			MT_RW_DATA);
...
void __dead2 scmi_main(bool primary_cold_boot,
			 struct ffa_boot_info_header *boot_info_header)
{
	...
	if (primary_cold_boot == true) {
		/* Clear BSS */
		memset((void *)SCMI_BSS_START,
		       0, SCMI_BSS_END - SCMI_BSS_START);

		/* Configure and enable Stage-1 MMU, enable D-Cache */
		scmi_plat_configure_mmu(ffa_id);
...
//	enable_fiq();
	...
	scmi_print_memory_layout(ffa_id);

	ffa_register_secondary_entrypoint(&secondary_cold_entry);
